...
@Component({
  ...
  styles: [`
    ...
    .calendar-day.other-month { background: var(--bg-tertiary); opacity: 0.6; }
    ...
    .day-task.urgent { background: var(--danger-bg); color: var(--danger-text); }
    .day-task.high { background: rgba(255, 71, 87, 0.15); color: #ff4757; }
    .day-task.medium { background: var(--warning-bg); color: var(--warning-text); }
    .day-task.low { background: var(--success-bg); color: var(--success-text); }
    ...
    @media (max-width: 768px) {
      ...
      .day-task { font-size: 9px; padding: 2px 4px; }
    }
  `]
})
export class CalendarPage implements OnInit {
  ...
  get calendarDays(): Array<{ day: number; isCurrentMonth: boolean; isToday: boolean; tasks: Task[] }> {
    ...
    const days: Array<{ day: number; isCurrentMonth: boolean; isToday: boolean; tasks: Task[] }> = [];
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push({
        ...
        tasks: this.tasks.filter(t => t.dueDate && t.dueDate.startsWith(dateStr))
      });
    }
